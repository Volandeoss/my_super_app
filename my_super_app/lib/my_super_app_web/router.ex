defmodule MySuperAppWeb.Router do
  ...
  scope "/admin", MySuperAppWeb do
    ...
    live_session :superadmin,
      root_layout: {MySuperAppWeb.Layouts, :admin},
  ...
  scope "/admin", MySuperAppWeb do
    pipe_through [:browser, :require_authenticated_operator_or_role]

    live_session :operator_role,
      root_layout: {MySuperAppWeb.Layouts, :admin},
      on_mount: {MySuperAppWeb.UserAuth, :ensure_authenticated} do
      live("/posts", PostPage)
      live("/images", ImagePage)
    end
  end

  scope "/admin", MySuperAppWeb do
    pipe_through [:browser, :require_authenticated_operator_or_superadmin]

    live_session :sup_oper,
      root_layout: {MySuperAppWeb.Layouts, :admin},
      on_mount: {MySuperAppWeb.UserAuth, :ensure_authenticated} do
      live("/", AdminMainPage)
    end
